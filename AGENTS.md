...
- **Orphan-ADR semantics.** An ADR is orphaned if nothing references it — neither current work nor a record of completed work. In a batch cycle that's `QUEUE.md` (in flight) and `EVIDENCE.md` (completed, the durable ledger); interactively, judge it against the change in flight and the codebase as it stands. (Transmitted as a concept by the `decide` skill, not a CLI command — ADR-0011.)
