...
- **Workers scope to the work unit's explicit instructions.** The worker updated all files listed in the work unit (skills, AGENTS.md, DESIGN.md) but left stale `.loop/HANDOFF.md` references in `loop.sh` and `prompts/worker.md` because they weren't explicitly listed. Workers follow the unit's scope precisely — if a file needs updating, name it in the work unit.
