...
- A universal prose spec as source of truth → it becomes a second model that can rot beside implementation ([doc-rot](https://github.com/bermudi/AgenticWiki/blob/main/wiki/concepts/doc-rot.md))
- A mandatory durable canon for every capability → accumulates wrong information even when structurally valid ([doc-rot](https://github.com/bermudi/AgenticWiki/blob/main/wiki/concepts/doc-rot.md), [plan-disposability](https://github.com/bermudi/AgenticWiki/blob/main/wiki/concepts/plan-disposability.md))
...
This rejects universal spec authority, not durable contracts as a category. A host project may assign a public promise to an OpenAPI schema, protocol definition, compatibility policy, executable contract test, or another native record through operational context, an accepted ruling, or an established metadata convention. Nospec preserves that ownership without importing litespec's canon/delta/archive model. Work specs remain disposable; designation is never inferred from a filename or the `nospec: true` adoption marker (ADR-0025).
...
- **Plan as living document → plan as disposable.** ExecPlan's durability claim rests on the document being maintained in lockstep with the code. nospec's `QUEUE.md` is consumed then discarded; the restart surface is the repo's actual durable state—its authoritative records, including any project-owned contract records, plus `EVIDENCE.md`—not a maintained plan. If the ExecPlan drifts you restart from drift; if Nospec records disagree, their partitioned authority makes the mismatch explicit rather than letting the plan silently win (ADR-0014, ADR-0025). Different failure modes, different recovery stories.
